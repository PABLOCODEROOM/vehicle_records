...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($plate_number) || empty($owner_name) || empty($phone_number)) {
        ...
        $sql = "INSERT INTO vehicles (user_id, plate_number, make, model, year, color, vin, owner_name, phone_number, purchase_date, status) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        
        $stmt = mysqli_prepare($connect, $sql);
        mysqli_stmt_bind_param($stmt, "isssisssss", $user_id, $plate_number, $make, $model, $year, $color, $vin, $owner_name, $phone_number, $purchase_date, $status);
        
        if (mysqli_stmt_execute($stmt)) {
            $message = "✅ Vehicle added successfully!";
            $_POST = []; // Clear form
        } else {
            $error = "❌ Error adding vehicle: " . mysqli_error($connect);
        }
    }
}
?>
...
<head>
    ...
    <title>Add Vehicle - VehiclePro</title>
                            ...
                            <div class="form-group">
                                ...
                                <div style="display: flex; gap: var(--spacing-md);">
                                    <input 
                                        ...
                                        id="color" 
                                        name="color" 
                                        value="<?php echo htmlspecialchars($_POST['color'] ?? '#000000'); ?>"
                                    ...
                                    <input 
                                        ...
                                        style="flex: 1;"
                                        readonly
                                        value="<?php echo htmlspecialchars($_POST['color'] ?? 'Select Color'); ?>"
                                    >
                                </div>
                            </div>
